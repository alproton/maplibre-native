...
LineProgram::LayoutUniformValues LineProgram::layoutUniformValues(
    ...
    const std::array<float, 2>& pixelsToGLUnits,
    const float pixelRatio) {
    return makeValues<LineProgram::LayoutUniformValues>(properties, tile, state, pixelsToGLUnits, pixelRatio);
}

LineSDFProgram::LayoutUniformValues LineSDFProgram::layoutUniformValues(
    const style::LinePaintProperties::PossiblyEvaluated& properties,
    float pixelRatio,
    const RenderTile& tile,
    const TransformState& state,
    ...
    std::array<float, 2> scaleA{
        {1.0f / tile.id.pixelsToTileUnits(widthA, state.getIntegerZoom()), -posA.height / 2.0f}};

    std::array<float, 2> scaleB{
        {1.0f / tile.id.pixelsToTileUnits(widthB, state.getIntegerZoom()), -posB.height / 2.0f}};
...
LineGradientProgram::LayoutUniformValues LineGradientProgram::layoutUniformValues(
    ...
    const std::array<float, 2>& pixelsToGLUnits,
    const float pixelRatio) {
    return makeValues<LineGradientProgram::LayoutUniformValues>(properties, tile, state, pixelsToGLUnits, pixelRatio);
}
