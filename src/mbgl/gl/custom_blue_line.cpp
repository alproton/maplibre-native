...
#include <mbgl/shaders/gl/custom_blue_line.hpp>
...
UniqueProgram createShader(gl::Context& context) {
    using shader = shaders::ShaderSource<shaders::BuiltIn::CustomBlueLineProgram, gfx::Backend::Type::OpenGL>;
...
CustomBlueLine::CustomBlueLine(gl::Context& context_)
    : context(context_),
      program(createShader(context_)) {}

CustomBlueLine::~CustomBlueLine() noexcept {
    clearVertexBufferImpl();
}

void CustomBlueLine::clearVertexBufferImpl() {
    if (vbo == 0) {
        return;
    }
...
void CustomBlueLine::updateVertexBufferImpl(const std::vector<float>& vertices) {
    if (vertices.empty()) {
        return;
    }
...
void CustomBlueLine::drawImpl() {
    ...
    MBGL_CHECK_ERROR(glUniform4f(0, 1.f, 1.f, 0.f, .7f));

    MBGL_CHECK_ERROR(glLineWidth(7.0f));
