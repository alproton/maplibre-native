...
void CustomPuck::debugChargers(const TransformState& transform) {
    ...
    // auto tile = transform.screenCoordinateToTileCoordinate(ScreenCoordinate{0, 0}, transform.getIntegerZoom());
    // std::mt19937 gen(static_cast<uint32_t>(tile.p.x) ^ static_cast<uint32_t>(tile.p.y) ^
    ...
    for (int p = 0; p < pointCount * 3; p++) {
        ...
        if (vertexBuffer.size() >= pointCount) {
            break;
        }
    }

    constexpr double defaultIconPixelSize = 20;
