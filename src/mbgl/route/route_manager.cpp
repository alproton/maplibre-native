...
void RouteManager::finalizeRoute(const RouteID& routeID, const DirtyType& dt) {
    ...
    if (routeID.isValid() && routeMap_.find(routeID) != routeMap_.end()) {
        ...
        if (updateGradients) {
            std::map<double, mbgl::Color> innerGradientMap = route.getRouteSegmentColorStops(
        ...
        if (updateTrafficSegments) {
            auto toTrafficSegments = [](const std::vector<Route::SegmentRange>& ranges)
                -> std::vector<style::LineLayer::TrafficSegment> {
