    ...
    if (style_ != nullptr && dirty_) {
        ...
        for (const auto& iter : routeMap_) {
            ...
            if (style_->getSource(baseGeoJSONSourceName) == nullptr) {
                ...
                std::unique_ptr<GeoJSONSource> geoJSONsrc = std::make_unique<GeoJSONSource>(
                    baseGeoJSONSourceName, mbgl::makeMutable<mbgl::style::GeoJSONOptions>(std::move(opts)));
            ...
            if (style_->getLayer(baseLayerName) == nullptr) {
                std::unique_ptr<style::LineLayer> baselayer = std::make_unique<style::LineLayer>(baseLayerName,
                                                                                                 baseGeoJSONSourceName);
        ...
        for (auto& iter : routeMap_) {
            ...
            if (style_->getSource(activeGeoJSONSourceName) == nullptr) {
                ...
                opts.lineMetrics = true;
                std::unique_ptr<GeoJSONSource> geoJSONsrc = std::make_unique<GeoJSONSource>(
                    activeGeoJSONSourceName, mbgl::makeMutable<mbgl::style::GeoJSONOptions>(std::move(opts)));
                geoJSONsrc->setGeoJSON(featureCollection);
            ...
            if (style_->getLayer(activeLayerName) == nullptr) {
                std::unique_ptr<style::LineLayer> activelayer = std::make_unique<style::LineLayer>(
                    activeLayerName, activeGeoJSONSourceName);
