...
public class EGLConfigChooser {
  ...
  public EGLConfig chooseConfig(EGLDisplay display) {
    ...
    int[] numConfigs = getNumberOfConfigurations(display, configAttribs);
    if (numConfigs[0] < 1) {
      Logger.e(TAG, "eglChooseConfig() returned no configs.");
    }

    // Get all possible configurations
    EGLConfig[] possibleConfigurations = getPossibleConfigurations(display, configAttribs, numConfigs);
  ...
  @NonNull
  private int[] getNumberOfConfigurations(EGLDisplay display, int[] configAttributes) {
    int[] numConfigs = new int[1];
    if (!EGL14.eglChooseConfig(display, configAttributes, 0 , null, 0, 0, numConfigs, 0)) {
      Logger.e(TAG, String.format(
        MapLibreConstants.MAPLIBRE_LOCALE, "eglChooseConfig(NULL) returned error %d", EGL14.eglGetError())
      );
    }
    return numConfigs;
  }
  ...
  @NonNull
  private EGLConfig[] getPossibleConfigurations(EGLDisplay display, int[] configAttributes, int[] numConfigs) {
    EGLConfig[] configs = new EGLConfig[numConfigs[0]];
    if (!EGL14.eglChooseConfig(display, configAttributes, 0, configs, 0, configs.length, numConfigs, 0)) {
      Logger.e(TAG, String.format(
        MapLibreConstants.MAPLIBRE_LOCALE, "eglChooseConfig() returned error %d", EGL14.eglGetError())
      );
    }
    return configs;
  }
  ...
  private EGLConfig chooseBestMatchConfig(EGLDisplay display, EGLConfig[] configs) {
    ...
    for (EGLConfig config : configs) {
      ...
      if (configOk) {
        ...
        boolean isCaveat = caveat != EGL14.EGL_NONE;
  ...
  private int getConfigAttr(EGLDisplay display, EGLConfig config, int attributeName) {
    int[] attributevalue = new int[1];
    if (!EGL14.eglGetConfigAttrib(display, config, attributeName, attributevalue, 0)) {
      Logger.e(TAG, String.format(
        MapLibreConstants.MAPLIBRE_LOCALE, "eglGetConfigAttrib(%d) returned error %d", attributeName, EGL14.eglGetError())
      );
    }
  ...
  private int[] getConfigAttributes() {
    ...
    return new int[] {
            ...
            EGL14.EGL_ALPHA_SIZE, translucentSurface ? 8 : 0,
            EGL14.EGL_DEPTH_SIZE, 16,
            EGL14.EGL_STENCIL_SIZE, 8,
            (emulator ? EGL14.EGL_NONE : EGL14.EGL_COLOR_BUFFER_TYPE), EGL14.EGL_RGB_BUFFER,
