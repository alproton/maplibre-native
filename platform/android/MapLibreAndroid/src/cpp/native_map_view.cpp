...
void NativeMapView::jumpTo(jni::JNIEnv& env,
    ...
    if (padding) {
        ...
        options.padding = mbgl::EdgeInsets{
            padding.Get(env, 0), padding.Get(env, 1), padding.Get(env, 2), padding.Get(env, 3)};
    }
    if (pitch != -1) {
        options.pitch = pitch;
    }
...
jni::Local<jni::Object<CameraPosition>> NativeMapView::getCameraForLatLngBounds(
    jni::JNIEnv& env,
...
jni::Local<jni::Object<CameraPosition>> NativeMapView::getCameraForGeometry(
    jni::JNIEnv& env,
    const jni::Object<geojson::Geometry>& jGeometry,
    double top,
    double left,
    double bottom,
    double right,
...
void NativeMapView::setPitch(jni::JNIEnv&, jni::jdouble pitch, jni::jlong duration) {
    map->easeTo(mbgl::CameraOptions().withPitch(pitch), mbgl::AnimationOptions{mbgl::Milliseconds(duration)});
}
...
jint NativeMapView::routeQueryRendered(JNIEnv& env,
                                       ...
                                       jni::jint radius) {
    if (getRouteMgr()) {
        if (!getRouteMgr()->hasRoutes()) {
            return -1;
        }

        std::vector<RouteID> routeIDs = getRouteMgr()->getAllRoutes();
        ...
        for (const auto& routeID : routeIDs) {
            std::string baseLayer = getRouteMgr()->getBaseRouteLayerName(routeID);
            assert(!baseLayer.empty() && "base layer cannot be empty!");
            if (!baseLayer.empty()) {
                baseLayers.push_back(baseLayer);
                baseLayerMapCache[baseLayer] = routeID;
            }
            std::string baseSource = getRouteMgr()->getBaseGeoJSONsourceName(routeID);
        ...
        if (!maxRouteIDs.empty()) {
            int top = getRouteMgr()->getTopMost(maxRouteIDs);
            RouteID topRouteID;
...
jni::Local<jni::Array<jni::Object<geojson::Feature>>> NativeMapView::queryRenderedFeaturesForPoint(
    JNIEnv& env,
    jni::jfloat x,
    jni::jfloat y,
    ...
    return Feature::convert(env, rendererFrontend->queryRenderedFeatures(point, {layers, toFilter(env, jfilter)}));
}

jni::Local<jni::Array<jni::Object<geojson::Feature>>> NativeMapView::queryRenderedFeaturesForBox(
    JNIEnv& env,
    jni::jfloat left,
    jni::jfloat top,
    jni::jfloat right,
...
void NativeMapView::addLayer(JNIEnv& env, jlong nativeLayerPtr, const jni::String& before) {
    ...
    try {
        ...
        jni::ThrowNew(
            env, jni::FindClass(env, "org/maplibre/android/style/layers/CannotAddLayerException"), error.what());
    }
}
...
void NativeMapView::addLayerAbove(JNIEnv& env, jlong nativeLayerPtr, const jni::String& above) {
    ...
    if (index + 1 > layers.size()) {
        // Not found
        jni::ThrowNew(env,
                      jni::FindClass(env, "org/maplibre/android/style/layers/CannotAddLayerException"),
...
void NativeMapView::addSource(JNIEnv& env, const jni::Object<Source>& obj, jlong sourcePtr) {
    ...
    try {
        ...
        jni::ThrowNew(
            env, jni::FindClass(env, "org/maplibre/android/style/sources/CannotAddSourceException"), error.what());
    }
}
...
#define METHOD(MethodPtr, name) jni::MakeNativePeerMethod<decltype(MethodPtr), (MethodPtr)>(name)
    ...
    jni::RegisterNativePeer<NativeMapView>(
        ...
        METHOD(&NativeMapView::routeGetActiveLayerName, "nativeRouteGetActiveLayerName"),
        METHOD(&NativeMapView::routeGetBaseLayerName, "nativeRouteGetBaseLayerName"),
...
std::map<double, double> NativeMapView::convert(JNIEnv& env,
                                                const jni::Array<jdouble>& keys,
                                                const jni::Array<jdouble>& values) {
    std::map<double, double> ret;
    size_t keyslen = keys.Length(env);
    size_t valueslen = values.Length(env);
    assert(keyslen == valueslen && "invalid map data");
    if (keyslen == valueslen) {
        for (size_t i = 0; i < keyslen; i++) {
            double key = keys.Get(env, i);
            double value = values.Get(env, i);
            ret[key] = value;
        }
    }
...
std::shared_ptr<route::RouteManager> NativeMapView::getRouteMgr() {
    return mapRenderer.getRendererBackend().getImpl().routeManager;
}
...
jint NativeMapView::routeCreate(JNIEnv& env,
    ...
    if (!getRouteMgr()->hasStyle()) {
        getRouteMgr()->setStyle(map->getStyle());
    }

    assert(getRouteMgr()->hasStyle() && "style object has not been set");
    RouteID routeID;
    if (getRouteMgr()) {
        if (getRouteMgr()->hasStyle()) {
            using namespace mbgl::android::conversion;
            ...
            if (outerDynamicWidthZooms && outerDynamicWidths && innerDynamicWidthZooms && innerDynamicWidths) {
                routeOptions.outerWidthZoomStops = convert(env, outerDynamicWidthZooms, outerDynamicWidths);
                routeOptions.innerWidthZoomStops = convert(env, innerDynamicWidthZooms, innerDynamicWidths);
            }

            routeID = getRouteMgr()->routeCreate(linestring, routeOptions);
        }
    }
...
jni::Local<jni::String> NativeMapView::routeGetActiveLayerName(JNIEnv& env, const jint routeID) {
    ...
    if (getRouteMgr()) {
        layerName = getRouteMgr()->getActiveRouteLayerName(RouteID(routeID));
    }
...
jni::Local<jni::String> NativeMapView::routeGetBaseLayerName(JNIEnv& env, const jint& routeID) {
    std::string layerName;
    if (getRouteMgr()) {
        layerName = getRouteMgr()->getBaseRouteLayerName(RouteID(routeID));
    }
...
jni::Local<jni::String> NativeMapView::routesGetCaptureSnapshot(JNIEnv& env) {
    ...
    if (getRouteMgr()) {
        captureStr = getRouteMgr()->captureSnapshot();
    }
...
jboolean NativeMapView::routeDispose(JNIEnv& env, jint routeID) {
    if (getRouteMgr()) {
        return getRouteMgr()->routeDispose(RouteID(routeID));
    }
...
jboolean NativeMapView::routeSegmentCreate(JNIEnv& env,
    ...
    if (getRouteMgr()) {
        ...
        return getRouteMgr()->routeSegmentCreate(RouteID(routeID), rsegopts);
    }
...
jboolean NativeMapView::routeProgressSet(JNIEnv& env, jni::jint routeID, jni::jdouble progress) {
    if (getRouteMgr()) {
        return getRouteMgr()->routeSetProgressPercent(RouteID(routeID), progress);
    }
...
jdouble NativeMapView::routeProgressSetPoint(
    ...
    if (getRouteMgr()) {
        ...
        double percentage = getRouteMgr()->routeSetProgressPoint(
...
void NativeMapView::routeSegmentsClear(JNIEnv& env, jint routeID) {
    if (getRouteMgr()) {
        getRouteMgr()->routeClearSegments(RouteID(routeID));
    }
}

jboolean NativeMapView::routeSetVanishing(JNIEnv& env, jni::jint routeID) {
    if (getRouteMgr()) {
        return getRouteMgr()->setVanishingRouteID(RouteID(routeID));
    }
...
jint NativeMapView::routeGetVanishing(JNIEnv& env) {
    if (getRouteMgr()) {
        return getRouteMgr()->getVanishingRouteID().id;
    }
