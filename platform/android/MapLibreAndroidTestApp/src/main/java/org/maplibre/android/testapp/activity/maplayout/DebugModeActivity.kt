...
open class DebugModeActivity : AppCompatActivity(), OnMapReadyCallback, OnFpsChangedListener {
    ...
    override fun onMapReady(map: MapLibreMap) {
        ...
        maplibreMap.setStyle(
            ...
            mapView.setCustomDotsNextLayer("")
            ...
            mapView.setCustomDotsOptions(0, options)

            options.innerColor = Color.valueOf(0.8f, 0.2f, 0.1f, 1.0f)
            options.outerColor = Color.valueOf(0.1f, 0.2f, 0.7f, 1.0f)
            options.innerRadius = 9.0f
            options.outerRadius = 16.0f
            mapView.setCustomDotsOptions(1, options)
            ...
            mapView.setCustomDotsPoints(0, MultiPoint.fromLngLats(pointList))

            // mapView.clearCustomDotsVideoMemory()

            val pointList2: MutableList<Point> = mutableListOf()
            for (i in 1..10000) {
                pointList2.add(Point.fromLngLat(-76.0 - Random.nextDouble(0.0, 3.0), 35.0 + Random.nextDouble(0.0, 3.0)))
            }
            mapView.setCustomDotsPoints(1, MultiPoint.fromLngLats(pointList2))
